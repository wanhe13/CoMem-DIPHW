...
if __name__ == "__main__":
    ...
    rows = 3000
    cols = 1000
    avg_cells_per_module = int(cols / num_modules) + 3
    avg_genes_per_module = 50
    target_density = 0.03
    module_density = 0.3
    ...
    print(f'avg_cells_per_module: {avg_cells_per_module}')
    
    # Create output directory and set filename
    os.makedirs('results', exist_ok=True)
    filename = 'results/ClusteringPerformance_nmodules_5metrics.csv'
    ...
    G_ig.vs['name'] = list(G_cell.nodes())  # Ensure vertex names are set
    ...
    # Map vertex indices back to node names
    ...
    G_ig_HT.vs['name'] = list(G_cell_HT.nodes())  # Ensure vertex names are set
