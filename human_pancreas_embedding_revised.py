...
# Ensure functions.py is accessible
...
seq_data_name = 'human_pancreas'

file_suffix = '_revised'

human_pancreas = pd.read_csv('data/GSE85241_cellsystems_dataset_4donors_updated.csv', delimiter='\t', index_col=0)
human_pancreas = human_pancreas.dropna()
print("Any NA values:", human_pancreas.isna().any().any())

counts = human_pancreas.copy()
...
# 1. Identify the ERCC spike-ins in the data
ercc_genes = [gene for gene in counts.index if 'ERCC' in gene]

# 2. Calculate the proportion of ERCC reads for each cell (column)
counts.loc['ERCC_sum'] = counts.loc[ercc_genes].sum(axis=0)
counts.loc['total_sum'] = counts.sum(axis=0)
counts.loc['ERCC_proportion'] = counts.loc['ERCC_sum'] / counts.loc['total_sum']

# 3. Filter out low-quality cells (e.g., cells with >10% ERCC reads)
threshold = 0.1
cells_to_keep = counts.columns[counts.loc['ERCC_proportion'] < threshold].tolist()

# Remove columns used for filtering from the filtered DataFrame
df_filtered = counts[cells_to_keep].copy()
df_filtered.drop(['ERCC_sum', 'total_sum', 'ERCC_proportion'], axis=0, inplace=True)

# 4. Normalize the data using ERCC spike-ins
ercc_scaling_factors = df_filtered.loc[ercc_genes].sum(axis=0)
ercc_scaling_factors = ercc_scaling_factors.replace(0, np.nan)

# Normalize the entire dataset using the ERCC scaling factors
df_normalized_ERCC = df_filtered.div(ercc_scaling_factors, axis=1)

# 5. Exclude ERCC genes from downstream analysis
df_normalized_ERCC = df_normalized_ERCC.drop(ercc_genes, axis=0)

# step 1
normalization = 'ERCC'

if normalization == 'CPM':
    data_normalized_by_cells = normalize_matrix_cpm(df_normalized_ERCC).astype(float)
elif normalization == 'ERCC':
    data_normalized_by_cells = df_normalized_ERCC.copy()
